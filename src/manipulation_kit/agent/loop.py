...
def robot_facts(world: Any = None) -> str:
    ...
    from ..primitives.orientation import (JAW_OPEN_M,  # noqa: PLC0415
                                          graspable_width_m)
    opening = None
    for gripper in (getattr(world, "grippers", None) or {}).values():
        if gripper.open_gap_m is not None:
            ...
            break
    open_m = JAW_OPEN_M if opening is None else opening
    ...
    lines = [
        ...
        f"{graspable_width_m(opening) * 1000:.0f} mm across "
