...
FORMULATIONS = ("choice", "score", "grasp")
...
def to_choices(answers: Mapping[str, Mapping[str, Any]], *,
    ...
    if "where" in answers:
        return dict(answers["where"]["dist"])
...
class AskingJudge:
    ...
    def answers(self, image: Path, obj: str) -> Dict[str, Any]:
        ...
        state = STATE.format(object=obj.replace("_", " "))
    ...
    def __call__(self, look: ServoLook) -> Dict[str, float]:
        ...
        started = time.time()
        answers = self.answers(Path(look.image), look.object)
...
__all__ = ["ALL_VIEWS", "AskingJudge", "DEFAULT_VIEWS", "FORMULATIONS", "LABELS", "QUESTION",
