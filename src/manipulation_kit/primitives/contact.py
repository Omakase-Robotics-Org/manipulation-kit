...
def record_contacts(world: WorldView, reports: Any,
    ...
    leading fingertip, :data:`.orientation.TIP_BELOW_TOOL_M` past the
    ...
    for report in items:
        ...
        tip = (np.asarray(report.p_tool, dtype=float)
               + _unit(travel) * ap.TIP_BELOW_TOOL_M)
...
class Press(Primitive):
    ...
    def _geometry(self, world: WorldView):
        ...
        # the fingertips lead, so the TOOL point is TIP_BELOW_TOOL_M behind
        at_face = face - d * ap.TIP_BELOW_TOOL_M
