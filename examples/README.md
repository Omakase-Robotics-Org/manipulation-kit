...
| `jev_servo.py`, `jev_judge.py`, `jev_judge_server.py` | `astra_loop` with a `Servo`: Astra decides the verbs (System 2), Jev-Omni judges the wrist look (System 1) — the kit draws the jaw opening (the two pads at the current gap, at the object's depth) and the object's outline on the wrist photo, Jev answers where the object lies from the opening (letters), its depth against the fingers, occlusion, the jaw turn and the fit, and the kit retreats / turns / steps / approaches by a fixed priority. `--segment-url` takes the outline from `segment_server.py` (SAM 3 on a GPU host, `segmenter.py` is the client), `--object-shape tape=cylinder` names a declared shape; `jev_jaws_lab.py` measures the questions offline. `--judge-only` records and never steps; `--rejudge RUN_DIR` re-judges a live run's saved wrist photos offline; `jev_judge_server.py` serves the classifier over HTTP (`--judge-url`) for a robot that cannot hold it. `--dry-run --misplace-mm 40` runs the geometry stand-in with no model | `--judge jev` / the server: `huggingface_hub`, `torch`, `torchvision`, `transformers`, a CUDA GPU (~47 GB measured); live: `--snapshot-cmd` |
