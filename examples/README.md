...
| `jev_servo.py`, `jev_judge.py`, `jev_judge_server.py` | `astra_loop` with a `Servo`: Astra decides the verbs (System 2), Jev-Omni judges the wrist look (System 1) — the kit draws its projected box on the wrist photo, Jev says inside / sticks out left / right / above / below, the kit steps the hand. `--judge-only` records and never steps; `--rejudge RUN_DIR` re-judges a live run's saved wrist photos offline; `jev_judge_server.py` serves the classifier over HTTP (`--judge-url`) for a robot that cannot hold it. `--dry-run --misplace-mm 40` runs the geometry stand-in with no model | `--judge jev` / the server: `huggingface_hub`, `torch`, `torchvision`, `transformers`, a CUDA GPU (~47 GB measured); live: `--snapshot-cmd` |
