...
#: a 160 mm crate standing on the d1-2 wagon top between HOME and the cube
#: (160 rather than step 5's 100 mm: step 3's standoff waits 80 mm over the
#: cube's top plus the 29 mm tip lead, and the body-guard-only detour already
#: clears a 100 mm crate — the discrimination below needs a crate it does not)
CRATE = ObjectView("crate", p=(0.39, -0.20, 0.246), size=(0.04, 0.20, 0.16))
...
def test_a_transit_over_a_wagon_rises_above_it(d1_arm, monkeypatch):
    ...
    plan = Approach(object="cube", side="right", direction="down").plan(world, d1_arm)
    ...
    monkeypatch.setattr(verbs, "_scene_for",
                        lambda primitive, world, kin: SceneGate(()))
    old = Approach(object="cube", side="right", direction="down").plan(world, d1_arm)
    assert old.ok
...
def test_the_run5_horizontal_approach_on_the_d1_2_scene_is_refused_with_the_wagon_named(
    ...
    unnecessary."""
    world = _d1_2(d1_arm)
    blind = Grasp(object="cube", side="right", direction="forward").plan(world, d1_arm)
    ...
    error = Grasp(object="cube", side="right", direction="forward").plan(world, d1_arm)
    assert not error.ok
...
def test_the_grasp_descent_is_still_straight(d1_arm):
    ...
    """
    world = _crate_world(d1_arm, CRATE)
    plan = Grasp(object="cube", side="right", direction="down").plan(world, d1_arm)
    assert plan.ok, str(plan)
...
def test_the_droop_margin_is_a_typed_policy_on_the_kinematics(policy_restored):
    ...
    world = _d1_2(kin)
    plan = Grasp(object="cube", side="right", direction="down").plan(world, kin)
    assert plan.ok, str(plan)
